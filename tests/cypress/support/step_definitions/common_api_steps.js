...
Then("the response status should be {int}", (statusCode) => {
  cy.get("@response").then((res) => {
    if (res.status !== statusCode) {
      ...
      throw new Error(
        `Expected status ${statusCode} but got ${res.status}. Response body:\n${bodyText}`,
      );
    }
...
When("I capture an in-stock plant id as {string}", (alias) => {
  cy.get("@authToken", { log: false }).then((authToken) => {
    cy.request({
      method: "GET",
      url: "/api/plants",
      ...
      const inStock =
        list.find((item) => {
          const qty = item && (item.quantity ?? item.stock ?? item.available);
          return typeof qty === "number" && qty > 0;
        }) || list[0];

      if (!inStock || !inStock.id) {
        throw new Error("No plant with a valid id was found in /api/plants");
      }

      sharedState[alias] = inStock.id;
      cy.wrap(inStock.id).as(alias);
    });
  });
});
...
When(
  ...
  (method, url, alias, placeholder, body) => {
    ...
    const timestamp = Date.now().toString();
    let bodyWithReplacement = body.replace(
      new RegExp(`{${placeholder}}`, "g"),
      val,
    );
    bodyWithReplacement = bodyWithReplacement.replace(
      /{timestamp}/g,
      timestamp,
    );
...
Then("the response body should contain the correct sale details", () => {
  const expectedId = sharedState.saleId;
  cy.get("@response")
    .its("body")
    .then((body) => {
